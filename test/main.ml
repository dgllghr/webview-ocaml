let make_url n =
  String.concat "" [
    ...
    string_of_int n;
    "%22)%3B%0A%20%20%3C%2Fscript%3E%0A%3C%2Fbody%3E%0A%3C%2Fhtml%3E"
  ]
...
let () =
  ...
  let test_handler (h: WvHandle.t) msg =
    ...
    | Ok () -> output := Some (int_of_string msg)
  ...
  let wv = Webview.create ~handler:test_handler "webview-ocaml integration test" url 800 600 in
